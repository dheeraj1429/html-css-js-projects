<?php 
    ...
    $dataWallet = mysqli_query($conn,"SELECT wlt.*, us.name as userName, us.email as userEmail,pk.type,pk.name as packageName,wl.balance FROM `".$tblPrefix."wallet_transactions` wlt LEFT JOIN `".$tblPrefix."users` us ON wlt.user = us.id LEFT JOIN `".$tblPrefix."packages` pk ON wlt.package = pk.id LEFT JOIN `".$tblPrefix."wallet` wl ON wlt.user = wl.user_id");
?>
<!DOCTYPE html>
<html lang="en">
